...
class MVouchersViewController: UIViewController {
  ...
  override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
    ...
    if segue.identifier == "goToProduct" {
      ...
      let vc = segue.destination as? MProductVoucherViewController
      vc?.address = self.voucherViewModel.selectedVoucher?.address ?? ""
      vc?.voucher = self.voucherViewModel.selectedVoucher
      
    }else if segue.identifier == "goToVoucher" {
      let vc = segue.destination as? MVoucherViewController
      vc?.address = self.voucherViewModel.selectedVoucher?.address ?? ""
      vc?.voucher = self.voucherViewModel.selectedVoucher
    }
  }
